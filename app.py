...
def get_all_data(endpoint):
    ...
    while url:
        try:
            ...
            response.raise_for_status()  # Lanzará un error si la respuesta tiene un código de error.
            response_data = response.json()
            data.extend(response_data['results'])
            url = response_data.get('next')  # Obtiene la URL de la siguiente página, si existe.
